          ...
          handler: function () {
            ...
            if (row) {
              $.messager.confirm('system alert', 'Are you sure to reset the password of 【' + row.realName + "】 password as:：0000", function (r) {
                if (r) {
                  $.get("/system/inventory/password/reset", {id: row.id}, function (rsp) {
    ...
    function createForm(id) {
      var dialog = $("<div/>", {class: 'noflow'}).dialog({
        ...
        onLoad: function () {
          ...
          $("#inventory_userName").textbox({
            label: 'username：',
            required: true,
            validType: ['userName', 'length[6, 10]', "remote['/system/inventory/check','userName']"]
          })
        },
        buttons: [{
          iconCls: 'fa fa-save',
